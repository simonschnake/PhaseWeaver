...
import phase_weaver.core.policy as policy

# Algorithm/policy enums and their defaults are re-exported from core
# (core/policy.py) so that `core` stays pure (no app/Qt knowledge). Existing
# `from phase_weaver.app.config import PHASE_INIT_MODE` callers keep working.
PHASE_INIT_MODE = policy.PHASE_INIT_MODE
PHASE_INIT_DEFAULT = policy.PHASE_INIT_DEFAULT
RECONSTRUCTION_ALGORITHM = policy.RECONSTRUCTION_ALGORITHM
RECONSTRUCTION_ALGORITHM_DEFAULT = policy.RECONSTRUCTION_ALGORITHM_DEFAULT
RECON_TIME_CONSTRAINT = policy.RECON_TIME_CONSTRAINT
RECON_TIME_CONSTRAINT_DEFAULT = policy.RECON_TIME_CONSTRAINT_DEFAULT
RECON_FREQUENCY_CONSTRAINT = policy.RECON_FREQUENCY_CONSTRAINT
RECON_FREQUENCY_CONSTRAINT_DEFAULT = policy.RECON_FREQUENCY_CONSTRAINT_DEFAULT
RECON_STOP_CONDITION = policy.RECON_STOP_CONDITION
RECON_STOP_CONDITION_DEFAULT = policy.RECON_STOP_CONDITION_DEFAULT
...
class PLOT_LINE_MODE(Enum):
    ...
    CURRENT_REFERENCE = "CRISP reference current"
    MAG_INPUT = "Input |F|"
    MAG_RECON = "Recon |F|"
...
PLOT_LINE_MODE_DEFAULT = {
    ...
    PLOT_LINE_MODE.CURRENT_REFERENCE,
