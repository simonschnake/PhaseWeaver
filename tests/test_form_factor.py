...
def test_formfactor_clamps_magnitude(grid):
    ...
    ff = FormFactor(grid=grid, mag=mag, phase=phase, frequency_constraint=ClampMagnitude(eps=0.5))
...
def test_formfactor_apply_constraint(grid, gaussian_density):
    ...
    ff2 = FormFactor(grid=ff.grid, mag=ff.mag.copy(), phase=ff.phase.copy(), frequency_constraint=ClampMagnitude(eps=0.5))
